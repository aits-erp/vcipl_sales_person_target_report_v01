...
def execute(filters=None):
    ...
    columns    = get_columns(categories, filters)
    data       = get_data(filters, categories)
    data       = add_totals(data, categories)
    summary    = get_summary(data, categories)
...
# ─────────────────────────────────────────────
def add_totals(data, categories):
    if not data:
        return data
    ...
    grand = {
        ...
        "invoice_count":       0,
        "item_count":          0,
        "is_total_row":        1    # makes row bold in Frappe report
    }

    for cat in categories:
        safe = cat.replace(" ", "_").replace("-", "_")
        grand[f"{safe}_target"]   = 0
        grand[f"{safe}_achieved"] = 0

    for row in data:
        grand["total_achieved"] += flt(row.get("total_achieved"))
        grand["total_target"]   += flt(row.get("total_target"))
        grand["invoice_count"]  += flt(row.get("invoice_count"))
        grand["item_count"]     += flt(row.get("item_count"))

        for cat in categories:
            safe = cat.replace(" ", "_").replace("-", "_")
            grand[f"{safe}_target"]   += flt(row.get(f"{safe}_target"))
            grand[f"{safe}_achieved"] += flt(row.get(f"{safe}_achieved"))
...
def get_summary(data, categories):
    ...
    for row in data:
        if row.get("is_total_row"):
            # read from grand total row directly
            total_achieved = flt(row.get("total_achieved"))
            total_target   = flt(row.get("total_target"))
            total_invoice  = flt(row.get("invoice_count"))
            total_item     = flt(row.get("item_count"))
